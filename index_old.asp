...
function ClickRefresh(url,rf_rate) {
	if (document.all.cmdrefresh.value=="Refresh อัตโนมัติ") {
		document.all.cmdrefresh.value="หยุด Refresh อัตโนมัติ";
		rf_rate="0";
	} else {
		document.all.cmdrefresh.value="Refresh อัตโนมัติ";
		rf_rate="1";
	}
...
function closer() {
	...
	<%
	if Trim(Request("page"))<>"" and Trim(Request("page"))<>"signin.asp" and left(Trim(Request("page")),5)<>"index" Then
		'response.write " alert('This window is about to close.'); "
...
dim bMsg
	...
	if Session("uid")<>"0"  And Session("uid")<>"" Then '--jum เพิ่ม And Session("uid")<>""
