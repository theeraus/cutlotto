...
<%
	...
	'// ถ้ามาจากการ click บันทึก
		...
		set objRS=objDB.Execute(SQL)
		SQL="delete tb_price_player from tb_price_player a  "
		SQL=SQL & " inner join sc_user b on a.player_id=b.user_id and dealer_id=" & dealer_id 
		SQL=SQL &  " where b.create_by_player=" & player_id  & " and game_type =" & game_type 
		objDB.Execute(SQL)
		for i=1 to 8
			play_type=i
			pay_amt =Request("p" & i)
			...
			objDB.Execute(SQL)
			'SQL="delete tb_price_player_Level2 where dealer_id="	 & dealer_id & " and create_by_player=" & player_id & " and game_type=" & game_type & " and play_type=" & play_type
			'objDB.Execute(SQL)
...
%>
	<form name="form1" action="price_player_config_byuser.asp" method="post">
	<center><br>
	<table width="500"  border="0" cellspacing="1" cellpadding="1">
		<tr height="35">
			...
			onClick="gotoPage('price_player_config_byuser.asp?player_id=<%=player_id%>&game_type=1');">		

			<img src="images/price_tos.jpg" border="0" style="cursor:hand;" name="pictureTos" 
			onMouseOver="change('images/price_tos_over.jpg',2);" 
			onMouseOut="change('images/price_tos.jpg',2)"	onClick="gotoPage('price_player_config_byuser.asp?player_id=<%=player_id%>&game_type=2');">	
			...
			onClick="gotoPage('price_player_config_byuser.asp?player_id=<%=player_id%>&game_type=3');">	-->
			<td>
		</tr>
	</table>
