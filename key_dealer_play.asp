		...
		%>
			...
			<table align="center" border="0">
				...
				<tr>
					...
					<td>
						<input type="button" class="inputM" name="submit2" style="cursor:hand;width:75;" value="ตกลง" onClick="click_login();">
					</td>
				</tr>
			</table>
			</form>
	...
	<div  class="col-md-3"  style="background: #fff;padding: 10px;">
			<div class="form-group row">
				<div class="col-lg-12 col-xl-6">
					...
					readonly="true"
					value='สมาชิก : <%=Session("logid") & " " & Session("uname") %>' />
				</div>
			</div>
			<div class="form-group">
				<label class="kt-font-danger">F2 = เลือกชื่อลูกค้า</label>
				<input type="hidden" name="player_id"  value="<%=player_id%>" />
				<input type="text" class="form-control" name="player_name"  readonly="true"
					onclick="SearchPlayer();"
					value="<%=player_name%>" 
					style="cursor:hand;" 
					onMouseOver="changeStyle(this,'form-control')"
					onMouseOut="changeStyle(this,'form-control')" />

			</div>
			<div class="form-group">
				<label class="kt-font-danger">F7 = ขณะนี้ใบที่</label>
				<input type="text" name="ticket_number" onKeyDown="chkEnterNumber(this);" class="form-control">
			</div>

			<table class="table  text-center">
				<tr class="btn-dark"><td colspan="2">ยอดใบนี้</td></tr>
				<tr><td colspan="2">ส่ง = <span id="send"></span> </td></tr>
				<tr><td colspan="2">รับแล้ว = <span id="receive"></span></td></tr>
				<tr><td colspan="2">รอรับ = <span id="wait"></span></tr>
				<tr><td colspan="2">เลขคืน = <span id="ret"></span></tr>
				<tr class="btn-danger" ><td colspan="2">ยอดรวมเทง = <%=formatnumber(total_play,0)%> </td></tr>
				<tr>
					<td> <% Response.Write " ip: " & Client_IP %></td>
					<td height="30"  align="right">
							<img src="images/upp.jpg" name="b_updown_type" style="cursor:hand;" onclick="click_updown_type_label()"></td>
					</tr>				
				<tr>
					<td colspan="2" align="right">
						<span id="wait_save" class="textbig_red"></span>
						<a href="key.html" target="_blank"><input type="button" class="inputE" value="วิธีกดแทงโพย" style="cursor:hand; width: 100px;"></a>
						<input type="button" class="inputR" value="F10=ส่ง" style="cursor:hand; width: 75px;" onClick="clicksubmit()">
					
					</td>
				</tr>
				<tr height="45">
					<td colspan="2" align="right">
						<div id="show_price"><% Call PrintPrice(Session("did"), player_id, Session("gameid"),"no","1") %></div>
					</td>
				</tr>
				<tr>
					<td colspan="2" align="right">
				...
				<tr>
					<td align="right"  colspan="4">
						<input type="button" class="inputM" value="โทรเพิ่มเครดิตเพิ่มแล้วกด" style="cursor:hand; width: 170px;"
			...
			<!--
				...
				<tr>
					...
					<td><input type="text" 
						...
						onKeyDown="chkEnter(this,1);" id="c12<%=right("00" & i,2)%>"   onClick="click_shwSign('c11',1);"></td>

					<td width="20" align="center" class="tdbody">=</td>
					<td><input type="text" style="width:98;" maxLength="14" class="input1"  name="key_money_col1<%=i%>" onKeyDown="chkEnter(this,2);" 
					...
					<td width="20" align="center" class="tdbody">=</td>
					<td><input type="text" style="width:98;" maxLength="14" class="input1" name="key_money_col2<%=i%>"  
					id="c23<%=right("00" & i,2)%>" onFocus="iBlur(this)" onKeyUp="pressPlus(this)" 
					onKeyDown="chkEnter(this,2);"
					onBlur="chkSum(this)" pattern="[0-9]*"></td>
					...
					<td width="20" align="center" class="tdbody">=</td>
					<td><input type="text" style="width:98;" maxLength="14" class="input1" name="key_money_col3<%=i%>" onKeyDown="chkEnter(this,2);"
  ...
  <div class="modal-dialog" role="document">
    ...
    <div class="modal-content">
		<div class="modal-header">
			<h5 class="modal-title">ค้นจาก ชื่อ หรือ นามสกุล</h5>
			<button type="button" class="close" data-dismiss="modal" aria-label="Close">
				<span aria-hidden="true">×</span>
			</button>
		</div>
      <div id="modalbody" class="modal-body">
        <iframe id="content_iframe" name="f_hidden" width="450" height="400" frameBorder="0"></iframe>
      </div>
...
function GetDiscount(play_type){
	...
	function GetPlayType_Money(obj1,obj2,obj3){
		...
		if ( (key_number.value.length)==1 && (updown_type.value=="ล" || updown_type.value=="บ+ล") && !isNaN(key_money) ){  // --- วิ่ง ล่าง
			play_type=6	
			money=key_money			
			return ChkMaxMoney(play_type,money,money_focus,key_number,key_money);
		}

		if ( (key_number.value.length)==1 && (updown_type.value=="บ" || updown_type.value=="บ+ล") && !isNaN(key_money) ){  // --- วิ่ง บน
			play_type=5	
			money=key_money
			return ChkMaxMoney(play_type,money,money_focus,key_number,key_money);
		}
		...
		//-- 125=100/200 -----
		if ( (key_number.value.length)==3 && updown_type.value=='บ+ล' && key_money.indexOf('/')>-1 && key_money.indexOf('*')==-1) {
			play_type=2  //--  3 บน		
			money=key_money.substring(0,key_money.indexOf('/')) //-- จำนวนเงินของตัวหน้า
			tmp1= ChkMaxMoney(play_type,money,money_focus,key_number,key_money);	
...
<script language="javascript">
    ...
    function SearchPlayer() {
		var url = "search_player_bydealer.asp?dealer_id=<%=dealer_id%>&game_type=<%=game_type%>&call_from=<%=call_from%>" ;
