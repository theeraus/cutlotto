...
<body topmargin="0"  leftmargin="0">
	...
	<table width="500"  border="0" cellspacing="1" cellpadding="1" bgcolor="#606060">
		...
		<tr>
			...
			<!--- ยอดแทง หัก ยอดถูก ----->
			<td class="tdbody1" bgcolor="#B3FFD9" colspan="2"  align="right"><b><%=FormatN(GetDiffTotal(user_id,this_loginid),0)%></b>&nbsp;&nbsp;&nbsp;</td>
		</tr>
		<tr>
...
'			If Len(Trim(Session("logid")))<8 Then
			If Len(Trim(this_loginid))<8 Then
			
				SQL="exec spGet_total_play_by_user_id " & user_id
			else
				SQL="exec spGet_total_play_level2 " & user_id
			end if
'response.write SQL & Session("logid")
'response.end
...
Function GetDiffTotal(u,this_loginid)
	Dim objRS1 , objDB1 , SQL
	set objDB1=Server.CreateObject("ADODB.Connection")       
	objDB1.Open Application("constr")
	Set objRS1 =Server.CreateObject("ADODB.Recordset")
